...
;; extra length of wallpaper handy in case of mistakes or miscalculations so he wants to buy a 
;; length 15% greater than the one he needs.
...
(defn get-measurement [side height]
  (* (/ side 0.52) (/ height 10)))

(defn wallpaper [l w h]
  (let [length (get-measurement l h)
        width (get-measurement w h)
        responses (responses)]
    (if
     (or (= 0 l) (= 0 w) (= 0 h)) "zero"
     (nth responses (Math/ceil (* (* (+ length width) 2) 1.15))))))